    ...
    try {
        ...
        Client.cmd.on('tiktok', async(data) => {
            if (isLimit(data.sender)) return data.reply(mess.limit) 
               ...
               Client.sendFileFromUrl(data.from, `${configs.apiUhy}/api/socialmedia/tiktoknowm?url=${data.body}&apikey=${configs.dapkey}`, 'tiktok.mp4', `Video Berhasil Di Dapatkan @${data.sender.split('@')[0]}`,  data.message)
        })
		...
		data.reply('https://bit.ly/3aoG44t Silahkan join untuk mendapatkan akses lebih')
		})
		Client.cmd.on('sewa', async (data) => {
		data.reply('*PRICE LIST*\n\n- 7HARI / *Rp 5.000*\n- 30HARI / *Rp 10.000*\n- Permanen?? *Gaada bang*\n\n*PAYMENT*\n\n- GOPAY/ DANA : *081252848955*\n- PULSA +5K : *081336990737*\n- QRIS ALL PAYMENT : *MINTA OWNER*\n\n*MINAT? PM*\nwa.me/6281252848955\nIG : instagram.com/naaaanndo')
		})
		/*ANIME*/
        ...
        Client.cmd.on('*', async (data) => {
            ...
            switch(command.toLowerCase()) {
				case 'self':
					...
					Client.self = true
					data.reply('Sukses merubah mode privat')
				break
				case 'public':
					if (!isOwner) return data.reply(mess.ownerOnly)
					if (!Client.self) return data.reply('Already Public Mode')
					Client.self = false
					data.reply('Sukses merubah mode publik')
				break
                case 'command':
                case 'cmd':
                case 'menu':
                ...
                case 'list':
                     ...
                     const buttonMessage = {
                           ...
                           footerText: 'Follow My IG - instagram.com/naaaanndo',
                                "contextInfo": {
